...
| NFR-002 | Loopback-only stays absolute, tested per surface: (a) `agenttalk dashboard --host ...` is rejected as an unknown option — no host surface exists on the alias at all; (b) `make_server` (and therefore `serve --host`) still refuses every non-loopback host; (c) the per-request loopback peer gate covers the new routes; (d) no write-method route exists (POST/PUT/DELETE/PATCH → 405 after the peer gate). | Proposed |
