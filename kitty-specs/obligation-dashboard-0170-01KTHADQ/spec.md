...
| FR-010 | Any bind failure (port in use, OS-denied port, or other socket error) exits with code 2 and an actionable message for both `serve` and `dashboard` — naming the host:port that failed and suggesting `--port 0` (ephemeral) or another port. Real-world repro to cover: Windows `WinError 10013` when the default port 8765 falls in a Windows excluded/reserved port range — today this escapes the command's error handling entirely and reaches the generic top-level handler with no guidance. On Windows the message points at reserved port ranges as a likely cause. | Proposed |
