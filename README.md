...
| `close {open,ack,draft,counter,check,publish,reopen,acceptance attach,acceptance successor,list,show}` | Aggregates gates + typed review evidence into one milestone/release verdict. Acceptance open adds `--acceptance-plan PLAN --project-repo PROJECT`; attach takes `--file BUNDLE --from ACTOR`; successor preserves a published parent under a new ID. `--scope-reduction` carries operator-approved scope reductions or exact gating-policy amendments; neither reports the original failure as passed. The [staged acceptance contract](docs/STEP-ACCEPTANCE-INC1.md) remains HOLD-only until final cold-review enforcement ships. |
