...
python -m pip install "git+https://github.com/zoolok17/agenttalk.git@v0.27.0"
...
Pin to a specific tag so you control upgrades. Replace `v0.27.0` with
...
| `agenttalk supervise (--init \| --report \| --plan \| --clear-restart)` | Thin support for the **external agent supervisor** (24/7 outage auto-restart). `--init` scaffolds `.agenttalk/supervisor.{json,ps1}` (a config you fill with per-agent launch commands + a generated PowerShell monitor script you run in its own window; a POSIX/bash supervisor is a follow-up — the Python core is already cross-platform). `--report` emits the read-only per-agent liveness JSON; `--plan` emits the **action plan** — the shared decision table the script executes. The ONLY auto-restart trigger is death of the process the supervisor launched; a stale heartbeat is a warn-only *suspect* signal (a healthy agent mid-long-turn doesn't heartbeat). Protected agents (`operator_facing` ∪ every active `role=lead`) are never auto-restarted (warn/note instead). |
| `agenttalk request-restart --for A [--from L] [--reason ...] [--force-protected]` | Queue a **manual** restart of agent `A`: writes an atomic, request-id-scoped `state/<A>.restart-request` marker the supervisor relaunches from and clears. Restarting a protected agent requires `--force-protected`. |
