...
| `agenttalk supervise (--init \| --report \| --plan \| --clear-restart)` | Thin support for the **external agent supervisor** (24/7 outage auto-restart). `--init` scaffolds `.agenttalk/supervisor.{json,ps1,sh}` (a config you fill with per-agent launch commands + a generated monitor script you run in its own window). `--report` emits the read-only per-agent liveness JSON; `--plan` emits the **action plan** — the shared decision table the script executes. The ONLY auto-restart trigger is death of the process the supervisor launched; a stale heartbeat is a warn-only *suspect* signal (a healthy agent mid-long-turn doesn't heartbeat). Protected agents (`operator_facing` ∪ every active `role=lead`) are never auto-restarted (warn/note instead). |
