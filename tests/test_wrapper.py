...
def test_run_wrapper_terminate_raising_does_not_replace_the_owner_exception(
    monkeypatch,
) -> None:
    """Round 11 connector finding: terminate() can itself raise
    (PermissionError on Windows, ProcessLookupError if the child already
    exited). Unguarded, that secondary error replaces the SystemExit/
    KeyboardInterrupt being cleaned up after, skips the bounded wait/kill
    fallback below it, and can leave the model child running - and under
    cmd_wrap converts an intended signal exit into the routine mapped-error
    exit code, corrupting the very termination facts this cleanup exists
    to preserve."""
    lines = [json.dumps(o) for o in CATALOG_TOOL]
    ...
    class _Popen:
        ...
        def wait(self, timeout=None):
            ...
            if self._returncode is None:
                raise run.subprocess.TimeoutExpired(cmd="codex", timeout=timeout)
            return self._returncode
