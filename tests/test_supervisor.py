...
def _write_versioned_python_stub(
    path: Path,
    log_path: Path,
    version: str,
) -> None:
    version_tuple = tuple(int(part) for part in version.split("."))
    probe_rc = 0 if version_tuple >= (3, 11) else 1
    if os.name == "nt":
        path.write_text(
            "@echo off\r\n"
            f'>>"{log_path}" echo %*\r\n'
...
@pytest.mark.parametrize(
    ("pinned", "reported_version", "expected"),
    [
        (True, "3.10", False),
        (True, "3.11", True),
        (False, "3.10", True),
    ],
    ids=["pinned-3.10", "pinned-3.11", "unset-3.10"],
)
def test_tool_runtime_preflight_requires_python_311_only_when_pinned(
    tmp_path: Path,
    pinned: bool,
    ...
    stub = tmp_path / f"python{suffix}"
    invocation_log = tmp_path / "python-invocations.log"
    _write_versioned_python_stub(stub, invocation_log, reported_version)
    config = {"tool_runtime_python": str(stub)} if pinned else {}
    (store.dir / "supervisor.json").write_text(
        json.dumps(config),
        encoding="utf-8",
    )
    ...
    result_path = tmp_path / "preflight-version.json"
    harness = "\n".join([
        "$ErrorActionPreference = 'Stop'",
        f"$Root = {_pslit(str(tmp_path))}",
        preflight_dependencies,
    ...
    harness_path = tmp_path / "preflight-version.ps1"
    harness_path.write_text(harness, encoding="utf-8-sig")
    ...
    version_probes = [line for line in invocations if line.startswith("-I -c")]
    assert len(version_probes) == (1 if pinned else 0)
    import_probes = [line for line in invocations if line == "-m agenttalk --version"]
    assert len(import_probes) == (0 if pinned and not expected else 1)
    diagnostics = completed.stdout + completed.stderr
    if pinned and not expected:
        assert "requires Python 3.11+" in diagnostics
        assert reported_version in diagnostics
    else:
        assert "requires Python 3.11+" not in diagnostics
...
def _tool_runtime_preflight_shells() -> list[object]:
    ...
    if pwsh := shutil.which("pwsh"):
        shells.append(pytest.param(pwsh, id="pwsh-7"))
    windows_powershell = (
...
def test_tool_runtime_preflight_executes_version_gate_in_each_windows_powershell(
    tmp_path: Path,
    shell: str,
) -> None:
    ...
    result_path = tmp_path / "supported-preflight.json"
    harness = "\n".join([
        "$ErrorActionPreference = 'Stop'",
        f"$Root = {_pslit(str(tmp_path))}",
        preflight_dependencies,
    ...
    harness_path = tmp_path / "supported-preflight.ps1"
    harness_path.write_text(harness, encoding="utf-8-sig")
    ...
    assert json.loads(
        result_path.read_text(encoding="utf-8-sig")
    )["ok"] is expected, completed.stdout + completed.stderr
    diagnostics = completed.stdout + completed.stderr
    if expected:
        assert "requires Python 3.11+" not in diagnostics
    else:
        ...
        assert "unavailable" not in diagnostics
