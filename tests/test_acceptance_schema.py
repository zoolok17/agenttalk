...
def test_inventory_comparisons_use_one_refusing_dispatch():
    ...
    counts = {"acceptance.py": 24, "acceptance_cold.py": 1,
              "acceptance_obligations.py": 1, "acceptance_history.py": 6, "close.py": 1}
    # 31 inventoried predicates; two compare both schema operands (33 calls).
    for filename, minimum in counts.items():
        ...
        assert len(calls) >= minimum, filename
        for call in calls:
