...
def _reduction(value):
    A._object(value, "rows reason alternatives impact owner expires_at cause evidence decision_ref", "scope reduction")
    ...
    if value["cause"] not in {"unavailable-tool", "measured-variance"}:
...
def _approval(store, data, parent, successor_id, plan_hash, reduction):
    ...
    if body != approval_payload(parent, successor_id, plan_hash, reduction):
...
def successor(store, *, parent_id, close_id, plan_file, project_repo, revision, by, at, reason,
    ...
    with close.close_transaction(store, parent_id) as transaction:
        ...
        amendment = {"schema_version": 1, "parent_close_id": parent_id,
...
def evaluate(store, record, plan, snapshot, *, depth=0):
    ...
    A._object(amendment, "schema_version parent_close_id parent_attempt_id successor_close_id prev_plan_hash "
              ...
              "reduction approval_hash", "amendment")
    A._version(amendment["schema_version"])
    ...
    snapshot["holds"].extend(h for h in inherited["holds"] if h[0] == "acceptance_scope_reduction_unapproved")
    ...
    if inherited.get("report_label"):
        ...
        snapshot["inherited_scope_reduction"] = inherited
        prior_rows = {row["id"]: row for row in inherited["outcomes"]}
        for outcome in snapshot["outcomes"]:
            prior = prior_rows.get(outcome["id"], {})
            if prior.get("disposition") == "scope-narrowed" and outcome["policy"] == "informational":
    ...
    if reduced or reduction is not None:
        try:
