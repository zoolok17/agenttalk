...
_BUS_COMMAND_CONTRACT = (
    ...
    "as `python -m agenttalk <subcommand> ...`. Treat agenttalk as the "
...
_DEFAULT_RULES = (
    ...
    "\n"
    "You MAY SEND (this is your job) with the sandbox-safe module form: "
    "`python -m agenttalk reply`, `python -m agenttalk send`, "
    "`python -m agenttalk escalate`, proposal-response / review-result via "
    "`python -m agenttalk reply --kind ...`, and "
    "`python -m agenttalk composing --to-request <id>` to mark a long draft "
    ...
    "`python -m agenttalk escalate --from <you>` with the decision, the options, "
    "and your recommendation, addressed to the liaison.\n"
    "\n"
    "Before any IRREVERSIBLE action tied to a tracked request (merge / release / "
    "deploy / delete), run "
    "`python -m agenttalk check --for <you> --to-request <id> --gates` FIRST: "
...
_CADENCE_RULES = (
    ...
    "(`python -m agenttalk reply` / `python -m agenttalk send` on the request_id) - "
    ...
    "`python -m agenttalk escalate` (or the liaison) as a controller-health / "
    "delivery problem - do NOT retry or reprocess the message yourself.\n"
    "\n"
    "You MAY SEND (this is the sweep's job) with the sandbox-safe module form: "
    "`python -m agenttalk reply`, `python -m agenttalk send`, "
    "`python -m agenttalk escalate`, proposal-response / review-result via "
    "`python -m agenttalk reply --kind ...`, and "
    "`python -m agenttalk composing --to-request <id>` for a long draft. You "
    "are HEADLESS: for a human decision run `python -m agenttalk escalate`, "
