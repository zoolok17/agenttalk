...
description: Enter listen mode as the codex agent — repeatedly wait for messages from the peer (typically claude) and handle them. Used when codex is the passive party — waiting for review requests, questions, wake signals, or cross-review of work the peer just finished.
---

# agenttalk-listen — Listen for peer messages (codex side)

You are the **`codex`** agent. Your peer (typically `claude`) is in
...
| `question`       | Answer directly via `agenttalk send --from codex --to claude --kind message -m "<answer>"`. |
