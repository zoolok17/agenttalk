...
- If the active assignment carries `lane_id`, run
  `python -m agenttalk lane workspace --id <lane_id>` and `cd` into that path
  before implementation. If no workspace resolves, STOP and tell the lead; never
  create or reuse your own worktree.
...
Both `implement` and `review` are equally normal actions. First **verify you own
THIS exact transition** for THIS WP from `spec-kitty next`/`status` - the EXACT
...
- **Wakes are latency optimization, not state.** If a wake is lost,
  the next poll catches the change (and Step 0.5 reconciles move/wake
  drift on start/rejoin by the transition key).
