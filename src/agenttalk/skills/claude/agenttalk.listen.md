...
heartbeat or a live waiter) and prints a free variant (e.g. `claude-2`).
If it refuses, **adopt the suggested name**: set
`$env:AGENTTALK_SELF = '<suggested>'` (and `$SELF`) before doing anything
else, then continue. Use `--json` if you want to parse `{refused,
active_holder, suggested}`.
...
- RECOVERY on the next invocation/rejoin: a kill loses only realtime
  ...
  is monotonic. Run `agenttalk sync --for $SELF` then `agenttalk threads
  --for $SELF` to catch up, `agenttalk recv --for $SELF` to inspect
  unread, and `agenttalk drain --for $SELF` ONLY when you intend to
  consume ALL unread (never blind-drain). Scoped-wait seen-state is not
  equivalent to having handled a thread.
...
still return promptly while an idle waiter costs almost nothing. You do
not normally need to tune these; the defaults are tuned for long idle
`listen`/`sk-loop` sessions.
...
- **Long-running work (#164).** If a message needs real time (a
  ...
  status via `agenttalk reply` — a reply is how you tell yourself the
  message is answered, and you will then stop, killing any of that turn's
  own background work still running. Instead run
  `agenttalk progress --from $SELF --to-id <id> -m "..."` as many times
  as useful. A progress note does NOT end your turn, does NOT close or
  advance anything (it never opens or derives a thread, and never counts
  as a reply to the opener), and does NOT mean you are done — keep
  working. Only your final `agenttalk reply` ends your turn.
