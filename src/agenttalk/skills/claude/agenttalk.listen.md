---
description: Enter listen mode as a Claude Code agent — repeatedly wait for messages from the peer and handle them. Used when this side is the passive party — waiting for review requests, proposals, questions, wake signals, or cross-review of work the peer just finished.
---
...
- exit 0: a message was received and printed. Classify and handle it
  (see below), then run `agenttalk threads --for $SELF` and resolve
  anything actionable before looping back.
