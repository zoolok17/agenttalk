...
3. **Send it.** Inside a managed wrapper turn
   (`$env:AGENTTALK_WRAPPER_GENERATION` is set), send with
   `--await-reply` and **return immediately to the wrapper**: it owns
   the inbox cursor and delivers the verdict in a later turn. Do not run
   `wait`, `sync`, `recv` or `drain` from that turn.
   ```powershell
...
5. **Act on it, then record it on the dispatch or proposal:**
   ...
   - `replace` / `defer` / `stop`: only the OPERATOR can override. Accept
     ...
     ```powershell
     agenttalk escalate --from $SELF --subject "challenge verdict: <verdict> on <outcome>" `
       --meta challenge=$reqId --meta challenge_verdict=<verdict> `
