...
#   .\supervisor.ps1 -Quiet          # run the loop silently (suppress the console log)
...
do {
  ...
  foreach ($name in $plan.agents.PSObject.Properties.Name) {
    ...
    # Console action log: ALWAYS surface a real action (relaunch/stuck_recover/
    # warn_only/suspect_warn/refuse_protected/clear_marker) with state+reason; a
    # steady no-action agent logs only when its state CHANGES (first sight +
    # transitions), so the window shows events without one HEALTHY line per poll.
    if (-not $Quiet) {
      if ($p.action -ne 'none') {
        Write-Host ("supervisor: {0}: {1} -> {2}: {3}" -f $name, $p.state, $p.action, $p.reason)
      } elseif ($lastLogged[$name] -ne $p.state) {
