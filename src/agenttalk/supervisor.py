  ...
  foreach ($name in $plan.agents.PSObject.Properties.Name) {
    ...
    switch ($p.action) {
      { $_ -in 'relaunch','stuck_recover' } {
        ...
        if (-not $seedOk) {
          if (-not $Quiet) { Write-Warning ("supervisor: {0}: seed/preflight failed - skipping relaunch this tick (fail closed)" -f $name) }
          ...
          if ($res) {
            ...
            if (-not $Quiet) { Write-Warning ("supervisor: {0}: {1} FAILED (no PID) - keeping marker/state for retry" -f $name, $p.action) }
          }
        }
      }
      ...
      'refuse_protected' { if (-not $Quiet) { Write-Warning ("supervisor: {0}: {1}" -f $name, $p.reason) }; if ($p.clear_marker) { & $AgenttalkCmd --root $Root supervise --clear-restart --for $name --request-id $p.clear_marker | Out-Null }; $state.agents.$name = $p.next_state }
      { $_ -in 'warn_only','suspect_warn','snapshot_unavailable' } {
        if (-not $Quiet) { Write-Warning ("supervisor: {0}: {1}" -f $name, $p.reason) }
