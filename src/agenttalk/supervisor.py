...
do {
  ...
  $plan = (& agenttalk --root $Root supervise --plan --state-file $StatePath --now $now --json) | ConvertFrom-Json
  foreach ($name in $plan.agents.PSObject.Properties.Name) {
    ...
    switch ($p.action) {
      'relaunch' {
        ...
        $newPid = Launch $name
        $ns = $p.next_state; if ($newPid) { $ns | Add-Member pid $newPid -Force; $ns | Add-Member pid_alive $true -Force }
...
# agenttalk supervisor (POSIX) — generated by `agenttalk supervise --init`.
# DECISION table is in Python (`agenttalk supervise --plan`); this script only
# tracks launched PIDs, runs the plan, and launches/kills processes.
#   ./supervisor.sh                # run the monitor loop
#   ./supervisor.sh --once --dry-run   # print the action plan once, change nothing
set -euo pipefail
HERE="$(cd "$(dirname "$0")" && pwd)"; ROOT="$(dirname "$HERE")"
CFG="$HERE/supervisor.json"; STATE="$HERE/supervisor-state.json"   # script-owned, not the bus
ONCE=0; DRY=0
for a in "$@"; do [ "$a" = "--once" ] && ONCE=1; [ "$a" = "--dry-run" ] && DRY=1; done
command -v jq >/dev/null || { echo "supervisor: needs jq"; exit 2; }
[ -f "$STATE" ] || echo '{"agents":{}}' > "$STATE"

pid_alive() { [ -n "${1:-}" ] && kill -0 "$1" 2>/dev/null; }

while :; do
  # 1) refresh launched-pid liveness for the planner
  for name in $(jq -r '.agents|keys[]' "$CFG"); do
    p=$(jq -r --arg n "$name" '.agents[$n].pid // empty' "$STATE")
    alive=false; pid_alive "$p" && alive=true
    tmp=$(jq --arg n "$name" --argjson a "$alive" '.agents[$n].pid_alive=$a' "$STATE"); echo "$tmp" > "$STATE"
  done
  # 2) ask Python for the safe action plan
  NOW=$(date +%s)
  PLAN=$(agenttalk --root "$ROOT" supervise --plan --state-file "$STATE" --now "$NOW" --json)
  for name in $(echo "$PLAN" | jq -r '.agents|keys[]'); do
    action=$(echo "$PLAN" | jq -r --arg n "$name" '.agents[$n].action')
    reason=$(echo "$PLAN" | jq -r --arg n "$name" '.agents[$n].reason')
    if [ "$DRY" = 1 ]; then echo "$name: $action — $reason"; continue; fi
    # (execute: relaunch/clear_marker/warn — operator fills launch.posix in supervisor.json)
    echo "$name: $action — $reason"   # see generated comments; mechanics mirror supervisor.ps1
  done
  [ "$ONCE" = 1 ] && break
  sleep "$(jq -r '.poll_seconds // 15' "$CFG")"
done
"""


def init(store: Store, *, force: bool = False) -> dict:
    """Scaffold the supervisor config + scripts under ``.agenttalk/``. Returns
    ``{path: "written"|"exists"}``. Never overwrites without ``force`` (the
    config especially — it holds the operator's filled-in launch commands)."""
    out: dict[str, str] = {}
    targets = {
        store.dir / "supervisor.json": CONFIG_TEMPLATE,
        store.dir / "supervisor.ps1": PS_TEMPLATE,
        store.dir / "supervisor.sh": BASH_TEMPLATE,
    }
