...
def _public_request_target(raw_target: str) -> str | None:
    if raw_target not in {PUBLIC_ROUTE, f"{PUBLIC_ROUTE}?beta=true"}:
        return None
    return raw_target
...
class GatewayFront:
        ...
        class Handler(BaseHTTPRequestHandler):
            ...
            def do_POST(self) -> None:
                ...
                if (
                    ...
                    or request_line[1] != self.path
                ):
                    self._reject_default()
                    return
                request_target = _public_request_target(self.path)
    ...
    def _proxy(
        ...
        body: bytes,
        request_target: str = PUBLIC_ROUTE,
        *,
        capability: str,
    ) -> None:
