...
**Status:** Revision 15 author fold against the panel over `f42570d..44b3787`,
...
and the issuer-extinction identity result over `b41b959`.
...
surface requires an operator other than the dead owner to select the source-
equal checkpoint, acknowledge that the prior effect is unknown
...
`ExactIssuerIdentityAdapterV1` lands and is reviewed. Revision 15 also reserves
“exact identity” for named identity types/adapters, uses “source-equal” for a
field copied from another record, “byte-identical” for serialization, “closed”
for an enumerated algebra, and “exactly one” only for cardinality. The attended
...
- **IMPLEMENTATION BLOCKED** means named executable prerequisites are absent.
  Revision 15 is blocked on #115, #146, #57,
...
The panel held Q1, Q2, and Q3 under attack; Revision 15 does not reopen those
...
| Q4-R11 | The ephemeral terminal variant carried request/action provenance and `next_entry` but did not explicitly repeat its final action-latch gate at dispatch. | **PRESENT AS A NORMATIVE CONTRACT; IMPLEMENTATION BLOCKED ON #146.** Its constructor captures a narrow final-action gate: dry-run false, kill switch clear, and enabled action-latch epoch. After the winning admission but before plan construction, the dispatcher requires that source-equal latch epoch under the read guard and separately rereads the kill switch. It retains the latch guard through issuance and preserves the native body's final kill-switch check. Independent post-construction latch and kill-switch races consume the call as `REJECTED_NO_EFFECT`, preserve `next_entry`, and create no plan, raw array, or effect. A third race flips the kill switch after outer validation/plan construction but before the inner check; the body returns typed invocation-bound `ACTIONS_DISABLED_NO_EFFECT` before raw-array materialization, the owner becomes `REJECTED_NO_EFFECT(FINAL_ACTION_GATE_CHANGED)`, and replay remains inert. |
| Q4-R12 | `DISPATCHER_SEAL_UNDELIVERED` had specified semantics but no named-result direction control, so an implementation could return another unavailability reason while generic no-effect controls remained green. | **PRESENT.** Mandatory controls stage valid Windows exact-FILETIME semantics with #146 absent and require the named `DISPATCHER_SEAL_UNDELIVERED` result before permit/provider evaluation, including retained-envelope retention and generic public attention. Linux/macOS require `EXACT_TARGET_EXECUTOR_UNAVAILABLE` precedence; a staged sealed Windows dispatcher with only the closure successor absent requires `SUCCESSOR_MISSING`. The controls state explicitly that these are prospective conformance results, not runtime facts at merged `587e7c1`. |
...
| Q4-R20 | The attended escape treated a generic issuer start token as if it were an exact-identity type, leaving `RECYCLED` without a defined comparator and allowing either duplicate authority or a permanent false hold. | **PRESENT — NARROWED TO DEFINITIVE PID ABSENCE; IMPLEMENTATION BLOCKED ON `ExactIssuerIdentityAdapterV1`.** `ConfiguredActionIssuerCheckpointV1` retains source-equal generic issuer PID/start values only as audit binding. Only a fresh independent OS query returning PID absent produces `PROVED_GONE { result: GONE }`; a present PID always produces `LIVE_OR_UNPROVEN`, whether the generic token is equal, different, truncated, or otherwise comparable. No `RECYCLED` disposal result exists. The attended commit rechecks absence while preserving the null-fence, current source-equal `RESERVED/PRE_BARRIER`, and source-equal current hold-source-hash gates, and remains non-destructive. Controls prove absent-PID admission, equal-token and different-token present-PID refusal, unavailable-observation refusal, and the inability of literal or tolerant generic-token comparison to authorize disposal. Suspected PID reuse is an operator-visible permanent V1 limitation until the named adapter lands. |
...
V1 limitations together. Q4 is **SPECIFIED; IMPLEMENTATION BLOCKED ON #115,
...
The eighth and final docs re-panel is scoped only to Revision 15's GONE-only
issuer-extinction boundary, exact-identity dependency, vocabulary correction,
and preserved replay/non-destructive gates, not a fresh open sweep. It may
