...
For persisted state, Revision 9 also reconstructs a typed exact-executor fact
from the current host at every entry. With state-loss quarantine `NONE`, Linux
and macOS retain any inherited
named reservation/phase, debt-only state, or retired-attempt cleanup
byte-identically before pre-barrier release, takeover, observation,
reconcile/release, finalization, or external call. Complete-extinction
quarantine retirement remains the separate platform-neutral no-OS-action
transition.
...
| Q4-R1 | The fresh refusal did not cover a Windows-created closure-bearing state resumed, reloaded, or inherited on a non-Windows host; reconcile/release or finalization could call externally or mutate debt there. | **PRESENT — RELEASE BLOCKER RESOLVED.** `ExactTargetExecutorCapabilityV1` is a typed, fresh current-host/action-site operand reused by fresh capability and every persisted-state entry. The module enumerates uninterrupted issuer, resume/re-reduction, reload/takeover, cross-platform inheritance, `PRE_BARRIER`, all external-effect phases/stages, debt-only `IDLE`, retired-attempt cleanup, malformed/quarantined state, and quarantine retirement. On Linux/macOS with quarantine `NONE`, every named reservation/phase, debt-only, or retired-cleanup entry retains all checked fields byte-identically before pre-barrier release, effect-guard acquisition, predecessor reasoning, takeover, capture, reconcile/release, finalization, `Stop-Tree`, or launch; it emits continuous `CAPABILITY_UNAVAILABLE`/`POLICY_HELD`. Unresolved quarantine uses its earlier fail-closed rows; complete-extinction quarantine retirement is explicitly outside named cleanup and remains a platform-neutral no-OS-action CAS. Module item 24 and core item 38 cross every entry archetype. |
