...
| Q8-R2 | Operators need advance documentation that fail-closed capability refusal can strand recovery indefinitely. | **PRESENT, WITH BOTH PERMANENT V1 REDUCTIONS NAMED TOGETHER.** 87-A implementation close and 87-C activation require reviewed 87-B/follow-up operator-manual and tutorial evidence that: (1) every Linux/macOS closure-dependent named teardown remains `CAPABILITY_UNAVAILABLE`/`POLICY_HELD` pending a human until a real exact-token adapter exists; and (2) automatic state-loss-quarantine retirement is unavailable on every platform in V1, so provenance loss remains `STATE_PROVENANCE_LOST`/`POLICY_HELD` pending attended handling. Every projection names the held agent and directs human action. |
...
removes unsafe automatic quarantine retirement from V1, and states both
permanent availability reductions together. A scoped Q4-only re-panel remains
