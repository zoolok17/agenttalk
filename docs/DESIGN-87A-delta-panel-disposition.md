...
**Status:** Revision 12 author fold against the panel over `f42570d..44b3787`,
...
`d67cef6`, plus the call-consumption result over `776578b`. This is a
...
and the closure successor remain required.

**Mode:** Reference.

**Audience:** 87-A Tier-3 reviewers and implementers of tasks #78, #115, #120,
and #146, plus the closure successor.
...
- **IMPLEMENTATION BLOCKED** means named executable prerequisites are absent.
  Revision 12 is blocked on #115, #146, and the closure successor.
...
| B2 | Linearize external calls armed by checked state and close both two-poller gaps. | **PRESENT AS A NORMATIVE CONTRACT; IMPLEMENTATION BLOCKED.** `ChildlessContinuationOwnerV1`, provider-version equality, and terminal retired-attempt IDs are defined in the module's closure-successor contract. [Action-time closure and closed dispatch](DESIGN-87A-owned-childless-wrapper-authority.md#action-time-closure-and-closed-supervisor-owned-tree-dispatch) requires one unique guard lineage per acquisition, atomic single-custody issuance, separate winning admission/plan-ownership/native-entry transitions, atomically admitted receipt consumption, total return-or-poison exception handling, `ARMED`/`CALL_RETURNED`, predecessor-death proof, and release-only late `HELD`. The closed dispatcher prevents raw entry plus call, admission, invocation, and receipt replay after #146. Exact transitions forbid reconciliation while a foreign continuation can resume and keep `STOP_TREE/ARMED` from proving or reissuing the effect. Merged #120 contributes target-local same-handle FILETIME execution/wait but no sealed dispatcher, POSIX executor, checked continuation owner, or action-scoped closure. Full effect linearization remains blocked on #115, #146, and the closure successor. |
...
| m10 | Qualify the two leading safety labels by their implementation dependencies. | **PRESENT.** The module's safety decision records merged #120 as a delivered ownership-input/target-local-effect dependency, while actual teardown remains implementation-blocked on #115, #146, and the closure successor. Both normative files state specification, delivery, conformance, and activation status separately. |
...
The panel held Q1, Q2, and Q3 under attack; Revision 12 does not reopen those
...
| ID | Panel result | Revision 12 disposition |
| --- | --- | --- |
| Q4 | The design projected a valid Linux exact token to `Stop-Tree`, but merged #120 only accepts that token as input and skips the corresponding target at execution. | **PRESENT — ORIGINAL POSIX BLOCKER RESOLVED; Q4 REMAINS IMPLEMENTATION BLOCKED.** The module deletes the non-Windows destructive mapping/type/join/projection and adds `EXACT_TARGET_EXECUTOR_UNAVAILABLE` to the static capability reasons. A valid Linux-token snapshot remains observation/barrier input; macOS has no admitted exact-token mapping. Without an admitted executor witness, no named POSIX childless effect graph is constructible. The mandatory control cites the actual Windows execution branch and non-Windows skip at `src/agenttalk/supervisor.py:8900-8932` rather than the input-acceptance lines. The Windows exact-FILETIME requirement remains absolute. Merged raw `Stop-Tree($targets)` separately means the cross-platform construction seal is not enforced until #146 lands and is reviewed. |
...
| Q4-R10 | Two aliases of one legitimate sealed call could dispatch concurrently because immutable/noncopyable calls had no atomic consumption admission; configured-agent and ephemeral calls could also replay sequentially because they had no lineage owner. | **PRESENT AS A NORMATIVE CONTRACT; IMPLEMENTATION BLOCKED ON #146.** Every closed dispatch variant now has exactly one private atomic use owner outside its deeply immutable call graph and exactly one opaque submission associating that exact call with that owner. CHILDLESS reuses its guard-lineage owner; each non-childless constructor atomically emits a dedicated owner. Submission aliases race `CALL -> DISPATCHING` for one admission; admission aliases race `DISPATCHING -> PLAN_OWNED` for one plan/invocation handle; invocation aliases race `PLAN_OWNED -> INVOKING` for one native entry. Normal return atomically yields one opaque receipt-custody handle associating the immutable receipt with that same owner; the receipt alone is inert. CHILDLESS custody-handle aliases race `RECEIPT -> PERMIT`, and non-childless aliases race `RECEIPT -> CONSUMING_RECEIPT` before planner behavior. Concurrent aliases and sequential replays at every boundary produce zero additional plan, mutation, launch, or effect. The admission and receipt-publication CAS operations atomically yield their preconstructed candidates; result-handoff failure, plan-handoff failure, normal receipt, positive no-effect rejection, uncertain native/receipt/planner outcome, receipt consumption, poison, and cleanup all have exact owner dispositions. A registry, caller mutex, call-ID set, or unstated dispatcher lock is not a construction proof. |
...
binds the prospective ordinary capture ID at begin. This revision is only a
**NORMATIVE-SPECIFICATION COMPLETE candidate /
IMPLEMENTATION BLOCKED** design. Merged raw `Stop-Tree($targets)` remains an
actual bypass, so Q4 and 87-A are not complete, conforming, sealed, or enforced
until #146 lands and is reviewed. A fifth scoped Q4-only re-panel may accept
the normative specification and its blocking decomposition; it cannot approve
