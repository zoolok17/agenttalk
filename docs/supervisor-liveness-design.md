...
`--snapshot-file` to its internal OS-bound executable poll (`supervise --plan`
remains the manual, non-advancing preview). It is an INPUT to the planner only; if
...
1. `Get-ProcSnapshot` — capture the snapshot, write it to a SEPARATE
   `supervisor-snapshot.json` and pass `--snapshot-file` to the executable poll
   whose caller is OS-verified as a live child of the selected PowerShell host
...
5. The do-loop still: refresh snapshot -> OS-bound executable poll -> switch on
