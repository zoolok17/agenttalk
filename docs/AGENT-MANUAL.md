...
7. **RELEASE RITUAL** - bump `src/agenttalk/__init__.py` + `pyproject.toml`, add a `CHANGELOG.md` section and `docs/ASSURANCE.md` entry, update every install pin and baseline, regenerate `docs/AGENTTALK-NEW-USER-MANUAL.pdf`, and `git commit -F <file>` (never `-m` for multi-line - PowerShell native-arg trap). An operator with repository workflow authority then dispatches `.github/workflows/release-provenance.yml` **from master** with the exact 40-character post-bump SHA plus X.Y.Z version. Use a fresh dispatch, never a partial rerun. Wait for the exact-SHA 12-leg gate and CodeQL, then follow the [carrier and inner-manifest verification procedure](DEV-GATE.md#verify-a-downloaded-provenance-artifact) and confirm that the 90-day-requested artifact contains the gate-built wheel/sdist bytes before manually tagging, pushing, and running `gh release create`. This read-only increment never tags or publishes. The project has one human: the later manual action is a deliberate second action in time, not self-review prevention or two-party control, and the workflow intentionally has no username allowlist. The deliberate release act may bump the release barrier via `close publish --bump-barrier` after a GO. If the bound barrier send or stamp fails, rerun that exact publish; never mint an ad hoc replacement barrier.
