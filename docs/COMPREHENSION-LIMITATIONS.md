...
| S8 | A BARE (unqualified) framework annotation match (`@RestController`, no dot) is accepted unconditionally, with no import/scope corroboration - a coarse-S1-evidence, PREVALENCE-judged acceptance (real, compiling Spring/JAX-RS code always carries the import or a wildcard import in practice), never a claim that this is structurally guaranteed. The named, accepted contrived case this prevalence judgment does not close: a file declaring its OWN unrelated annotation type with the identical simple name as a real framework one (e.g. its own bare, non-imported `@GetMapping`) publishes a phantom route exactly as confidently as the real thing. | Declared: `_annotation_qualifier_is_recognized`'s own docstring (adapters/java.py, MICRO-NOD 50b F4). | tests/test_comprehension_adapter_java.py (`test_a_bare_route_annotation_with_no_import_at_all_is_a_judged_limit_still_recognized`) | Yes - issue #141's explicit binding stage resolves a bare annotation reference against real import/scope evidence, closing the named phantom-collision case structurally rather than by prevalence judgment. |
...
| A3 | RE-BUCKETED from bucket B (comment 3884350384, reviewer-3's own triage-audit flip): a pom's own `<dependency>` coordinate declared 2+ times with DISAGREEING `<optional>`/`<scope>` metadata. `edge_id` does not hash `optional` at all (disagreeing-`<optional>` pair collided by construction, coalescing to a silently-picked value with no record) and DOES hash `phase` (disagreeing-`<scope>` pair never collided at all - TWO edges published, each looking like an independently real dependency). Confirmed real: Maven 3.9.11 builds this pom shape (warning only). | Now publishes exactly ONE edge per disagreeing coordinate (the first declaration's own values - never a guess at which Maven would "really" pick, just never two edges asserting two independently-real relationships that do not both exist), paired with a named `duplicate_dependency_coordinate` problem listing every disagreeing claim. A genuinely repeated, byte-identical declaration still coalesces silently (unaffected). | tests/test_comprehension_adapter_java.py (three new F8 tests) + tests/test_comprehension_scan_pipeline.py (one new end-to-end F8 test) | **Fixed - MICRO-NOD 50b F8, this same close-out commit (the PR's own last commit before merge).** |

### 3a-residual. Named residuals of the fixed bucket-A items (reviewer-3's own audit findings, safe-direction)

Two residuals reviewer-3's own confirmation-delta audit found in F5's and F6's own fixed behavior - neither is wrong-data-capable (both are honest misses or a malformed-input-only edge, never a false claim), so neither reopens F5/F6; recorded here rather than silently carried.

| # | Residual | Direction | Disposition |
|---|---|---|---|
| R1 | F5's own cross-file String-shadow retraction (worker.py) suppresses a `cli_main` claim whenever a real `String`-named type exists anywhere in-scan in the same package, regardless of whether THAT shadowing type itself sits under a recognized test source root. A real `src/main` launcher shadowed only by a `String` type declared under `src/test` is retracted (a real, valid `cli_main` silently not published) rather than kept. | Safe (a miss, never a false fact) - never publishes a wrong claim, only withholds a right one. | Not fixed further this round - the F7 machinery (this producer's own edge_phase/test-source-root check) already knows which root each file sits under, so narrowing this is a cheap, well-understood future follow-up, not a new mechanism to design. |
| R2 | F6's own fix (deriving package from the file's own real declaration) can still let a `src/main` class's bare reference resolve, at medium confidence, to a same-simple-name sibling declared ONLY under `src/test` in the identical package - this producer's own package derivation is agnostic to source-root phase. | Malformed-input-only (reviewer-3's own compile argument, recorded): a real Maven build's `src/main` compilation classpath never includes `src/test` classes at all, so a `src/main` reference to a `src/test`-only symbol could only arise from already-non-compiling source - not a realistic-input risk for genuinely valid Java. | Not fixed - recorded as a named, judged residual; genuinely malformed/non-compiling input is out of this producer's own correctness bar elsewhere too. |
...
\* B11: reviewer-3's own confirmation-delta attempted to independently reproduce this row and could not - its own "empty repo" fixture was not actually empty. Sweep-classified but NOT independently audited by reviewer-3, unlike every other row above (which the confirmation delta did verify). Recorded honestly rather than silently presented as equally confirmed.
...
- 8 pre-existing structural/provenance/coarse-evidence caveats (already declared, unaffected by this close-out - includes S8, the F4 bare-name judgment named explicitly per reviewer-3's own reworded framing: a prevalence-judged acceptance, never a structural guarantee).
- 3 pre-existing closed-vocabulary "unsupported shape" audit tables (~20 named shapes total).
- 31 distinct findings from this PR's own 223-comment cross-vendor sweep, after reviewer-3's own triage audit flipped one bucket-B row to bucket A: 3 wrong-data-capable (all three fixed - F6/F7/F8), 20 safe-direction (registered above, including two named residuals of the F5/F6 fixes reviewer-3's audit itself surfaced, and one row - B11 - honestly marked sweep-classified-but-not-independently-audited), 8 polish/declared-limit-adjacent (registered above).
