...
| --- | --- | --- |
| N01 | Grant automatic teardown authority over a positively owned wrapper only after two complete post-establishment CLI-child absence observations; target its complete nonce-anchored owned tree; and stop same-incumbent recovery after a hard attempt cap instead of fading into exponential backoff. | **PRESENT.** The [normative module](DESIGN-87A-owned-childless-wrapper-authority.md) exports `PROVABLY_CHILDLESS_OWNED_WRAPPER`; the [core](DESIGN-87A-supervisor-classifier-authority.md) suppresses generic teardown for the entire child-death-sourced subset and admits only that named proof. A nonrenewable same-turn establishment guard prevents two pre-handoff captures from counting. The actual checked-managed/observed-root nonce sources retain explicit provenance. Unmerged #120 candidate `28f663f` supplies only the mapped 64-entry owned-tree snapshot and deny-only post-kill launch barrier; the explicit closure successor supplies action closure and, with task #115, effect linearization. That successor does not exist as of 2026-07-31, so pre-reservation `CAPABILITY_UNAVAILABLE` holds the named path and keeps recovery `POLICY_HELD` pending a human. Fail-closed state-loss quarantine prevents cap/debt reset. Origin-neutral debt globally blocks relaunch after partial kill while immutable residual-subset authority keeps a rootless survivor recoverable. Automatic failures one/two remain continuously visible without generic backoff; the third—including an advertised provider's closure failure—becomes continuously exhausted, and uncertain closure release retains every fence. The [delta disposition](DESIGN-87A-delta-panel-disposition.md) accounts for the eleven review findings; 87-B owns human delivery. |
