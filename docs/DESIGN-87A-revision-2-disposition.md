...
| A12 | Give all classifier mutation one checked observation delta. | **PRESENT (replacement).** `ClassifierObservationDeltaV1` owns only the ordinary poll projection: freshness, continuity, child counters, poll identity, and absence evidence. The task #115 owner accepts one closed `ClassifierStateDeltaV1` tag per transaction; private non-childless authority and witness/permit-bound childless-effect variants own consumed IDs and execution fencing. An observation commits first, then any later authority/effect delta is reconstructed against its successor revision. |
...
| A29 | Preserve no-new-daemon, persistence-plane, or runtime-dependency scope. | **PRESENT — DECIDED CONSTRAINT (operator, 2026-07-31; M5 Option A).** The promise is absolute: implementation is pure code over existing checked state plus transient caller-owned synchronization, `dependencies = []` remains hard, and there is no daemon, new persistence plane, durable helper or OS object, runtime dependency, or mechanism-specific/separately versioned exception. Merged #120 at `587e7c1` does not prove action-scoped creation closure, has no POSIX exact-token executor, and supplies no trustworthy host/process-universe token. Persisted evidence is inert: without a fresh non-serializable witness matching its executor binding, no executor-dependent external effect and no authority-enabling or effect-owned mutation is constructible. The availability cost has two permanent V1 forms. Linux/macOS closure-dependent named teardown remains `CAPABILITY_UNAVAILABLE`/`POLICY_HELD` pending a human until a reviewed exact-token adapter exists. Automatic state-loss-quarantine retirement is unavailable on every platform, so provenance loss remains `STATE_PROVENANCE_LOST`/`POLICY_HELD` pending attended handling. PID and start, hostname, state_epoch, process_source_digest, MachineGuid alone, local absence are insufficient. A future retirement successor may read only an existing OS token and may not add a file, registry value, helper, daemon, OS object, persistence plane, or dependency. An unprovable implementation case creates a task, not a mechanism. |
...
| --- | --- | --- |
| N01 | Grant automatic teardown authority over a positively owned wrapper only after two complete post-establishment CLI-child absence observations; target its complete nonce-anchored owned tree; and stop same-incumbent recovery after a hard attempt cap instead of fading into exponential backoff. | **PRESENT.** The [normative module](DESIGN-87A-owned-childless-wrapper-authority.md) exports `PROVABLY_CHILDLESS_OWNED_WRAPPER`; the [core](DESIGN-87A-supervisor-classifier-authority.md) suppresses generic teardown for the entire child-death-sourced subset and admits only that named proof. A nonrenewable same-turn establishment guard prevents two pre-handoff captures from counting, and the actual checked-managed/observed-root nonce sources retain explicit provenance. Merged #120 at `587e7c1` supplies the strict mapped 64-entry owned tree, exact same-handle Windows identity-check/termination for an openable matching target, a conditional bounded wait attempt after successful termination, a recycle-aware deny-only barrier, exact attended-reset evidence, and a request-bound attended archive. It has neither a POSIX exact-token executor nor a trustworthy host/process-universe token. The named effect path is structural rather than entry-enumerated: persisted evidence carries an inert executor binding, while every effect-bearing target, reservation, call, receipt, and authority/effect mutation requires a fresh matching witness-derived permit. Deserialized or future state can recover evidence but cannot manufacture the object needed to act. The two resulting permanent V1 holds are explicit: Linux/macOS closure-dependent named teardown remains `CAPABILITY_UNAVAILABLE`/`POLICY_HELD` pending a human, and automatic state-loss-quarantine retirement is unavailable on every platform pending attended handling. #120 also does not freeze child creation or own attempt-bound continuation/reconciliation; the explicit closure successor supplies those semantics and, with task #115, full effect linearization. Origin-neutral debt globally blocks relaunch after partial kill while immutable residual-subset authority keeps a rootless survivor recoverable. Automatic failures one/two remain continuously visible without generic backoff; the third—including an advertised provider's closure failure—becomes continuously exhausted, and uncertain closure release retains every fence. The [delta disposition](DESIGN-87A-delta-panel-disposition.md) explicitly withdraws Revision 9's whole-state byte-identity claim, accounts for the structural Q4 replacement, and preserves the approved Q9 evidence; 87-B owns human delivery. |
...
| B07 | Project status, doctor, attention, and web diagnostics. | **DEFERRED to 87-B.** Every unavailable-capability projection must join the exact fingerprint, name the held agent, and state that operator action is required; a bare enum is nonconforming. Operator-facing surfaces must distinguish both permanent V1 reductions: Linux/macOS closure-dependent named teardown is `CAPABILITY_UNAVAILABLE`/`POLICY_HELD`, and automatic state-loss-quarantine retirement is unavailable on every platform with `STATE_PROVENANCE_LOST`/`POLICY_HELD` pending attended handling. |
...
| C03 | Define dormant release parity, activation unit, controlled restart, and no partial 87-A-without-87-B activation. | **DEFERRED to 87-C.** Activation also requires task #114, matching-generation 87-B projections, and reviewed 87-B/follow-up operator-manual and tutorial evidence naming both permanent V1 reductions: Linux/macOS closure-dependent named teardown remains `CAPABILITY_UNAVAILABLE`/`POLICY_HELD` pending a human, and automatic state-loss-quarantine retirement is unavailable on every platform so provenance loss remains `STATE_PROVENANCE_LOST`/`POLICY_HELD` pending attended handling. The evidence must name the held agent and required human action; task #115 precedes durable 87-A state. |
...
replaces entry-point enumeration with a fresh-witness construction boundary and
records the POSIX named-teardown and V1 automatic-quarantine-retirement
reductions together. Neither withdrawal reopens the original 59-ID split
disposition.
