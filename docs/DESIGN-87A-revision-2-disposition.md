...
This audit does not mark 87-A delivered or conforming. Revision 11 of the atomic
normative specification is a **NORMATIVE-SPECIFICATION COMPLETE candidate /
IMPLEMENTATION BLOCKED** on #115, #146, and the closure successor.
...
| A12 | Give all classifier mutation one checked observation delta. | **PRESENT (strengthened replacement).** The caller-supplied delta is removed. Task #115 remains the sole owner: it accepts one opaque, one-shot, deeply immutable ordinary-observation receipt and derives the private observation mutation—freshness, continuity, child counters, poll identity, and absence evidence—inside the same checked RMW transaction. Callers cannot supply confirmation successors. Private non-childless authority and witness/permit-bound childless-effect variants own consumed IDs and execution fencing. Observation commits first; later authority/effect construction uses the successor revision. |
...
| --- | --- | --- |
| N01 | Grant automatic teardown authority over a positively owned wrapper only after two complete post-establishment CLI-child absence observations; target its complete nonce-anchored owned tree; and stop same-incumbent recovery after a hard attempt cap instead of fading into exponential backoff. | **PRESENT AS A NORMATIVE CONTRACT; IMPLEMENTATION BLOCKED.** The module exports the named proof and the core suppresses generic teardown for the child-death-sourced subset. #115 privately derives two-observation confirmation from distinct sealed receipts; callers cannot forge the successor. Merged #120 supplies the strict 64-entry snapshot and exact target-local Windows FILETIME behavior, but no POSIX executor, trustworthy process-universe token, action-scoped closure, or sealed effect entry. Revision 11 requires one unique guard lineage, transitively immutable private effect graphs, and #146's closed dispatcher over independently authorized childless/configured-agent/ephemeral variants. Merged raw `Stop-Tree($targets)` means Q4 and 87-A are not complete, conforming, sealed, or enforced in code until #146 lands and is reviewed. The three permanent V1 capability limitations, origin-neutral debt, three-attempt cap, retained closure fencing, and approved Q9 evidence remain unchanged; 87-B owns human delivery. |
...
boundary. Revision 11 closes its permit-adjacent specification holes with a
unique guard lineage, transitively sealed private values, #115 owner-private
observation reduction, and the #146 closed-dispatch contract while explicitly
recording that the runtime seal is not delivered. It also
...
residual. Neither withdrawal reopens the original 59-ID split disposition. This
audit is complete; 87-A delivery/conformance remains blocked on #115, #146, and
the closure successor.
