...
| A29 | Preserve no-new-daemon, persistence-plane, or runtime-dependency scope. | **PRESENT — DECIDED CONSTRAINT (operator, 2026-07-31; M5 Option A).** The promise is absolute: implementation is pure code over existing checked state plus transient caller-owned synchronization, `dependencies = []` remains hard, and there is no daemon, new persistence plane, durable helper or OS object, runtime dependency, or mechanism-specific/separately versioned exception. Incomplete/incompatible #120 snapshot evidence refuses observation. Merged #120 at `587e7c1` does not prove action-scoped creation closure and has no POSIX exact-token executor. With quarantine `NONE`, fresh pre-reservation `CAPABILITY_UNAVAILABLE` creates no destructive tuple, reservation, attempt, debt, or call, while a persisted reservation/phase, debt-only state, or retired cleanup inherited on an unavailable host is retained byte-identically before any cleanup call or mutation. Both perform no closure-dependent named teardown and keep recovery `POLICY_HELD` pending a human. The separate complete-extinction quarantine retirement is no-OS-action global state-loss handling, not named cleanup. Structural unavailability never becomes `CLOSURE_VETOED`, retry, or exhaustion. An unprovable implementation case creates a task, not a mechanism. |
...
| --- | --- | --- |
| N01 | Grant automatic teardown authority over a positively owned wrapper only after two complete post-establishment CLI-child absence observations; target its complete nonce-anchored owned tree; and stop same-incumbent recovery after a hard attempt cap instead of fading into exponential backoff. | **PRESENT.** The [normative module](DESIGN-87A-owned-childless-wrapper-authority.md) exports `PROVABLY_CHILDLESS_OWNED_WRAPPER`; the [core](DESIGN-87A-supervisor-classifier-authority.md) suppresses generic teardown for the entire child-death-sourced subset and admits only that named proof. A nonrenewable same-turn establishment guard prevents two pre-handoff captures from counting. The actual checked-managed/observed-root nonce sources retain explicit provenance. Merged #120 at `587e7c1` supplies the strict mapped 64-entry owned tree, exact same-handle Windows identity-check/termination for an openable matching target, a conditional bounded wait attempt after successful termination, a recycle-aware deny-only barrier, exact attended-reset evidence, and a request-bound attended archive. It has no POSIX exact-token executor. With quarantine `NONE`, accepted Linux observation tokens—and every other fresh non-Windows named path—receive independent pre-reservation `CAPABILITY_UNAVAILABLE(EXACT_TARGET_EXECUTOR_UNAVAILABLE)` and construct no destructive tuple/authority, while any named reservation/phase, debt-only state, or retired cleanup inherited on Linux/macOS is retained byte-identically by the fresh current-host executor gate before takeover, capture, reconcile/release, finalization, or external call. Both unavailable named paths keep recovery `POLICY_HELD` pending a human; no previously held teardown becomes executable solely because #120 merged. The separate complete-extinction quarantine retirement is global no-OS-action state-loss handling. #120 also does not freeze child creation or own attempt-bound continuation/reconciliation; the explicit closure successor supplies those semantics and, with task #115, full effect linearization. Origin-neutral debt globally blocks relaunch after partial kill while immutable residual-subset authority keeps a rootless survivor recoverable. Automatic failures one/two remain continuously visible without generic backoff; the third—including an advertised provider's closure failure—becomes continuously exhausted, and uncertain closure release retains every fence. The [delta disposition](DESIGN-87A-delta-panel-disposition.md) accounts for the eleven review findings and scoped post-panel correction; 87-B owns human delivery. |
...
| B07 | Project status, doctor, attention, and web diagnostics. | **DEFERRED to 87-B.** Every `CAPABILITY_UNAVAILABLE` projection must join the exact fingerprint, name the held agent, and state that operator action is required; a bare enum is nonconforming. |
