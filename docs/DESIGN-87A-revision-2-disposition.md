...
| --- | --- | --- |
| N01 | Grant automatic teardown authority over a positively owned wrapper only after two complete post-establishment CLI-child absence observations; target its complete nonce-anchored owned tree; and stop same-incumbent recovery after a hard attempt cap instead of fading into exponential backoff. | **PRESENT.** The [normative module](DESIGN-87A-owned-childless-wrapper-authority.md) exports `PROVABLY_CHILDLESS_OWNED_WRAPPER`; the [core](DESIGN-87A-supervisor-classifier-authority.md) suppresses generic teardown for the entire child-death-sourced subset and admits only that named proof. A nonrenewable same-turn establishment guard prevents two pre-handoff captures from counting. The actual checked-managed/observed-root nonce sources retain explicit provenance. Merged #120 at `587e7c1` supplies the strict mapped 64-entry owned tree, exact same-handle Windows identity-check/termination for an openable matching target, a conditional bounded wait attempt after successful termination, a recycle-aware deny-only barrier, exact attended-reset evidence, and a request-bound attended archive. It still does not freeze child creation or own attempt-bound continuation/reconciliation; the explicit closure successor supplies those semantics and, with task #115, full effect linearization. Therefore pre-reservation `CAPABILITY_UNAVAILABLE` still holds every closure-dependent named path and keeps recovery `POLICY_HELD` pending a human; no previously held teardown becomes executable solely because #120 merged. Fail-closed state-loss quarantine prevents cap/debt reset. Origin-neutral debt globally blocks relaunch after partial kill while immutable residual-subset authority keeps a rootless survivor recoverable. Automatic failures one/two remain continuously visible without generic backoff; the third—including an advertised provider's closure failure—becomes continuously exhausted, and uncertain closure release retains every fence. The [delta disposition](DESIGN-87A-delta-panel-disposition.md) accounts for the eleven review findings; 87-B owns human delivery. |
